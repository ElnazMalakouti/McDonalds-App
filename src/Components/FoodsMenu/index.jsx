...
const FoodsMenu = () => {
    ...
    return (
        <div className="w-[58%] max-h-[84vh]  m-4 flex flex-col rounded-[1rem] bg-[#F0F8FF] justify-center items-center gap-3">
            ...
            <div className="w-full p-2 gap-4 overflow-auto scrollbar-thin scrollbar-thumb-gray-500 flex flex-row flex-wrap justify-center items-center">
                {foodlist.map(item => {
                    return <div key={item.id} className='w-[45%]'>
                        <FoodCard                        
                        id={item.id}
                        name={item.name}
                        price={item.price}
